...
namespace HRIS.Application.Services
{
    public class LeaveRequestService:ILeaveRequestService
    {
        ...
        public async Task<IEnumerable<object>> GetWorkflowProcessesForFollowUp()
        {
            // Get the current user from HttpContextAccessor
            var userName = _httpContextAccessor.HttpContext?.User.Identity?.Name;
            if (string.IsNullOrEmpty(userName))
            {
                throw new UnauthorizedAccessException("User not authenticated.");
            ...
            var userRoles = await _userManager.GetRolesAsync(user);
            var applicationsToFollowUp = new List<LeaveRequest>();
            var request = _httpContextAccessor.HttpContext.Request;
            var baseUrl = $"{request.Scheme}://{request.Host}/Uploads/";

            foreach (var role in userRoles)
            {
                if (role == "Employee Supervisor" || role == "HR Manager")
                {
                    // Fetch leave requests assigned to the role that are pending action
                    var roleApplications = await _leaveRequestRepository.GetAllToStatusAsync(role);
                    applicationsToFollowUp.AddRange(roleApplications);
                }
                else if (role == "Employee")
                {
                    // Fetch leave requests where the current user is expected to respond or provide input
                    var userApplications = await _leaveRequestRepository.GetAllByUserAsync(r => r.EmployeeId == user.Id);
                    applicationsToFollowUp.AddRange(userApplications);
                }
            }

            // Prepare the final list of applications that need follow-up
            var followUpApplications = applicationsToFollowUp.Select(app => new
            {
                RequestId = app.RequestId,
                RequestName = app.RequestName,
                ProcessId = app.ProcessId,
                ...
                Status = app.Process?.Status,
                CurrentStep = app.Process?.CurrentStep?.StepName ?? "Unknown",
                RequiresActionBy = app.Process?.CurrentStep?.RequiredRole != null
                    ? app.Process.CurrentStep.RequiredRole.Name
                    : "Unknown",
                FileName = app.FileName,
                FilePath = app.FileName != null ? $"{baseUrl}{Uri.EscapeDataString(app.FileName)}" : null
            }).ToList();

            return followUpApplications;
        }
    }
}
